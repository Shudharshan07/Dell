  ...
  return (
    <div className="h-full overflow-y-auto bg-[#EDEDEB]">
      <div className="mx-auto max-w-5xl space-y-4 p-6">
        <div className="flex flex-col gap-3 rounded-xl border border-[#D1CFCA] bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-[#6B7280]">Templates with <code className="rounded bg-[#EAE8E3] px-1">{"{{variable}}"}</code> detection, live preview, and Send-to-Playground.</p>
            </div>
          </div>
        ...
        {!toolsetId ? (
          <div className="rounded-xl border border-dashed border-[#D0CECA] py-12 text-center text-xs text-[#787670]">Create a toolset first.</div>
        ) : (
          <>
              ...
              <div className="grid grid-cols-1 gap-3 rounded-xl border border-[#D1CFCA] bg-white p-4 lg:grid-cols-2">
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {formVars.length === 0 ? <span className="text-[11px] text-[#9CA3AF]">No variables detected.</span>
                      : formVars.map((v) => <span key={v} className="rounded-full bg-indigo-50 px-2 py-0.5 font-mono text-[10px] font-semibold text-indigo-700">{`{{${v}}}`}</span>)}
                  </div>
                ...
                <div className="space-y-2 rounded-lg border border-[#E5E7EB] bg-[#FAFAFA] p-3">
                  <p className="text-[10px] font-bold uppercase tracking-wide text-[#787670]">Live preview</p>
                  {formVars.map((v) => (
                    <Input key={v} placeholder={v} value={values[v] ?? ""} onChange={(e) => setValues((s) => ({ ...s, [v]: e.target.value }))} className="bg-white" />
                  ))}
                  <pre className="mt-1 whitespace-pre-wrap rounded border border-[#D0CECA] bg-white p-2 font-mono text-[11px] text-[#111827]">{render(form.content, values) || "—"}</pre>
                </div>
              </div>
            )}
            ...
            <div className="space-y-3">
              {items.length === 0 && !creating ? (
                <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-[#D0CECA] py-10 text-center text-xs text-[#787670]"><FileCode className="size-5" /> No prompts yet.</div>
              ) : items.map((it) => (
                <div key={it.id} className="rounded-xl border border-[#D1CFCA] bg-white p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      <button onClick={() => setActiveId(activeId === it.id ? null : it.id)} className="rounded-lg px-2 py-1 text-[11px] font-semibold text-indigo-700 transition hover:bg-indigo-50">{activeId === it.id ? "Close" : "Fill & preview"}</button>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {(it.variables ?? detectVars(it.content)).map((v) => <span key={v} className="rounded-full bg-indigo-50 px-2 py-0.5 font-mono text-[10px] font-semibold text-indigo-700">{`{{${v}}}`}</span>)}
                  </div>

                  {activeId === it.id && (
                    <div className="mt-3 space-y-2 rounded-lg border border-[#E5E7EB] bg-[#FAFAFA] p-3">
                      {activeVars.map((v) => (
                        <Input key={v} placeholder={v} value={savedValues[v] ?? ""} onChange={(e) => setSavedValues((s) => ({ ...s, [v]: e.target.value }))} className="bg-white" />
                      ))}
                      <pre className="whitespace-pre-wrap rounded border border-[#D0CECA] bg-white p-2 font-mono text-[11px] text-[#111827]">{filled || "—"}</pre>
                      <div className="flex justify-end">
